...
const elements = {
    ...
    legacyMigrationModal: document.getElementById('legacy-migration-modal'),
    legacyMigrationCount: document.getElementById('legacy-migration-count'),
    legacyMigrateYesBtn: document.getElementById('legacy-migrate-yes-btn'),
    legacyMigrateSkipBtn: document.getElementById('legacy-migrate-skip-btn'),
...
async function initializeFirebase() {
    ...
    try {
        ...
        // Keep the user signed in across browser sessions
...
function setupEventListeners() {
    ...
    elements.legacyMigrateYesBtn.addEventListener('click', handleLegacyMigrationYes);
    elements.legacyMigrateSkipBtn.addEventListener('click', handleLegacyMigrationSkip);
...
function setAuthLoading(isLoading) {
    ...
    elements.authSubmitBtn.classList.toggle('is-loading', isLoading);
...
function toggleAuthMode() {
    ...
    elements.authSubtitle.textContent = authMode === 'signup'
        ? 'Create an account to store your notes securely'
        : 'Sign in to access your notes';
    elements.authToggleText.textContent = authMode === 'signup'
        ? 'Already have an account?'
        : "Don't have an account?";
    elements.authToggleBtn.textContent = authMode === 'signup' ? 'Sign in' : 'Create account';
...
async function showApp() {
    ...
    // Check for legacy shared-collection migration
    await checkLegacyMigration();

    // Start listening to Firestore
    startRealtimeSync();
    startPdfsSync();
    if (typeof startTabsSync === 'function') startTabsSync();
}
...
async function handleMigrationYes() {
    try {
        ...
        const targetCol = notesCol();
        if (!targetCol) return;
        
        for (const note of localNotes) {
            const docRef = note && note.id
                ? targetCol.doc(String(note.id))
                : targetCol.doc();
...
function notesCol() {
    ...
    return db.collection('users').doc(auth.currentUser.uid).collection('notes');
}
...
async function checkLegacyMigration() {
    ...
    try {
        ...
        if (profile.exists && profile.data().legacyMigrated) return;

        const legacySnapshot = await db.collection('notes').get();
        if (legacySnapshot.empty) {
            await profileRef.set({ legacyMigrated: true }, { merge: true });
            return;
        }

        elements.legacyMigrationCount.textContent = legacySnapshot.size;
        elements.legacyMigrationModal.classList.add('open');
    } catch (error) {
        // If the legacy collection is not readable (e.g. rules already tightened), skip quietly.
        console.warn('Legacy migration check skipped:', error.message);
    }
}

async function handleLegacyMigrationYes() {
    if (!db || !auth || !auth.currentUser) return;

    const uid = auth.currentUser.uid;
    try {
        updateSyncStatus('syncing');
        elements.legacyMigrationModal.classList.remove('open');
        showToast('Moving your old notes...');
        ...
        legacySnapshot.docs.forEach(doc => {
            const data = doc.data();
            batch.set(targetCol.doc(doc.id), {
                title: data.title || '',
        ...
        await batch.commit();
        await db.collection('users').doc(uid).set({ legacyMigrated: true }, { merge: true });

        showToast('✅ Notes moved to your private account!', 'success');
    } catch (error) {
...
function handleLegacyMigrationSkip() {
    elements.legacyMigrationModal.classList.remove('open');
    showToast('Migration skipped. Old notes left in place.');
}
...
function startRealtimeSync() {
    ...
    try {
        unsubscribeListener = notesCol().orderBy('updatedAt', 'desc')
            .onSnapshot((snapshot) => {
